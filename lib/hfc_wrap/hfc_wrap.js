...
const transErrHeadLen = 'Transaction returned with failure:'.length
...
var invokeChaincode = function(peerNames, channelName, chaincodeName, fcn, args, username, org, eventHubTimeout) {
    ...
    .then((results) => {
        ...
        for (var i in proposalResponses) {
            ...
            if (proposalResponses && proposalResponses[i].response && proposalResponses[i].response.status === 200) {
                ...
                if (!errDetails) //目前只保留一条错误信息
                    errDetails = proposalResponses[i].details
            }
        ...
        if (all_good) {
            ...
            if (errDetails)
                return Promise.reject(util.format('invoke response:%s', errDetails));
            else
    ...
    .then((results) => {
        ...
        for (var i in proposalResponses) {
            ...
            if (proposalResponses && proposalResponses[i].response && proposalResponses[i].response.status === 200) {
                ...
                //details: 'Transaction returned with failure: XXXXXXX.'
                if (!errDetails) //目前只保留一条错误信息
                    errDetails = proposalResponses[i].details.substr(transErrHeadLen + 1)
            }
            all_good = all_good & one_good;
        }
        if (all_good) {
            ...
            if (errDetails)
                return Promise.reject(util.format('install response:%s', errDetails));
            else
...
var __instantiateOrUpdateChaincode = function(type, channelName, chaincodeName, chaincodeVersion, functionName, args, username, org, eventHubTimeout) {
    ...
    .then((results) => {
        ...
        if (all_good) {
            ...
            if (errDetails)
                return Promise.reject(util.format('%s response:%s', type, errDetails));
            else

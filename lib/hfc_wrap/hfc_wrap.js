...
/********************************  Query  ************************************************/
/*****************************************************************************************/
var queryChaincode = function(peer, channelName, chaincodeName, args, fcn, username, org, errMsgJson) {
    var target = peer ? helper.buildTarget(peer, org) : undefined

    return helper.getUserClientAndChannel(username, org)
    .then((obj) => {
            var channel = obj.chan
            var client = obj.client
            if (channelName && channel.getName() != channelName) {
        ...
        (err) => {
            if (err == helper.ErrorUserNotExists)
                return Promise.reject(err)

            return Promise.reject(logger.errorf('Failed to get submitter %s, err=%s', username, err))
        })
    .then((response_payloads) => {
            ...
            if (response_payloads) {
                ...
                if (qResult instanceof Error) {
                    var errDetails = null
                    if (qResult.details)
                        errDetails = __getErrMsgFromInvokeResponse(qResult.details)
                    else
                        errDetails = qResult.toString()
                    ...
                    if (errDetails.length > 0 && errMsgJson == true) {
                        try {
                            ...
                            logger.error('queryChaincode: parse errDetails to JSON failed, errDetails=[%s] err=%s', errDetails, err)
                        }
                    }
...
var getChaincodes = function(peer, type, username, org, channelName) {
    var target = peer ? helper.buildTarget(peer, org) : undefined;
    var channel 
    var client 

    return helper.getOrgAdminClientAndChannel(org)
    .then((obj) => {
            client = obj.client
            channel = obj.chan
            if (channelName && channel.getName() != channelName) {
...
var getChannels = function(peer, username, org) {
    ...
    return helper.getUserClient(username, org)
    .then((client) => {
...
/********************************  Invoke  ***********************************************/
/*****************************************************************************************/
var invokeChaincode = function(peerNames, channelName, chaincodeName, fcn, args, username, org, newUserWhenNil, errMsgJson, eventHubTimeout) {
    ...
    return helper.getUserClientAndChannel(username, org, newUserWhenNil)
    .then((obj) => {
            client = obj.client
            channel= obj.chan
            if (channelName && channel.getName() != channelName) {
    ...
    .then((results) => {
        ...
        /*
        var rwset = getInvokeRWSet(proposalResponses[0].payload)
        logger.debug('invokeChaincode: rwset=', rwset)
        */
                ...
                if (!errDetails) { //目前只保留一条错误信息
                    ...
                    if (errDetails.length > 0 && errMsgJson == true) {
                        try {
                            ...
                            //如果转换JSON失败，仍然返回字符串格式，这里记录个日志即可
                            logger.error('invokeChaincode: parse errDetails to JSON failed, errDetails=[%s] err=%s', errDetails, err)
                        }
                    }
                }
            }
        ...
        if (all_good) {
            ...
            */
            ...
            var eventhubs = helper.newEventHubs(peerNames, org, client);
...
var installChaincode = function(peers, chaincodeName, chaincodePath, chaincodeVersion, username, org) {
    ...
    .then(()=>{
        return helper.getOrgAdminClient(org)
        .then((client) => {
                var request = {
...
var __instantiateOrUpdateChaincode = function(peers, type, channelName, chaincodeName, chaincodeVersion, functionName, args, username, org, cctimeout, eventHubTimeout) {
    ...
    .then((results) => {
        ...
        if (all_good) {
            ...
            var eventhubs = helper.newEventHubs(peers, org, client);
            for (let key in eventhubs) {
                let eh = eventhubs[key];
                eh.connect();
...
var instantiateChaincode = function(peers, channelName, chaincodeName, chaincodeVersion, functionName, args, username, org, cctimeout, eventHubTimeout) {
    if (!functionName)
        functionName = 'init'
    
    return __instantiateOrUpdateChaincode(peers, 'instantiate', channelName, chaincodeName, chaincodeVersion, functionName, args, username, org, cctimeout, eventHubTimeout)
}
var upgradeChaincode = function(peers, channelName, chaincodeName, chaincodeVersion, functionName, args, username, org, cctimeout, eventHubTimeout) {
    if (!functionName)
        functionName = 'upgrade'

    return __instantiateOrUpdateChaincode(peers, 'upgrade', channelName, chaincodeName, chaincodeVersion, functionName, args, username, org, cctimeout, eventHubTimeout)
}
...
var createChannel = function(channelName, channelConfigPath, username, orgName) {
    ...
    return helper.getOrgAdminClient(orgName)
        .then((client) => {
            ...
            (err) => {
                return Promise.reject(logger.errorf('getOrgAdminClient Failed, Error: %s ', err))
            })
        .then((result) => {
...
var joinChannel = function(channelName, peers, username, org, eventWaitTime) {
    ...
    .then((obj) => {
        ...
        logger.debug('received member object for admin of the organization "%s": ', org);
        tx_id = client.newTransactionID();
        let request = {
            txId :  tx_id
        };
    ...
    .then((genesis_block) => {
        ...
        eventhubs = helper.newEventHubs(peers, org, client);
        for (let key in eventhubs) {
            let eh = eventhubs[key];
            eh.connect();
        }
...
var _proposalRspProto = grpc.load(__dirname + '/protos/peer/proposal_response.proto').protos;
var _proposalProto = grpc.load(__dirname + '/protos/peer/proposal.proto').protos;
var _RWSetProto = grpc.load(__dirname + '/protos/ledger/rwset/rwset.proto').rwset;
var _KVRWSetProto = grpc.load(__dirname + '/protos/ledger/rwset/kvrwset/kv_rwset.proto').kvrwset;
function getInvokeRWSet(data) {
    var payload =_proposalRspProto.ProposalResponsePayload.decode(data)
    var extension = payload.getExtension()
    
    var chaincodeAction = _proposalProto.ChaincodeAction.decode(extension)
    var result = chaincodeAction.getResults()
    
    var txReadWriteSet = _RWSetProto.TxReadWriteSet.decode(result)
    var ns_rwset = txReadWriteSet.get_ns_rwset()
    
    for (var i in ns_rwset) {
        var ns = ns_rwset[i].namespace
        var rwset = ns_rwset[i].rwset
        
        var kvrwset = _KVRWSetProto.KVRWSet.decode(rwset)
        console.log("namespace=%s, rwset=",ns, kvrwset)
    }
...
exports.userEnroll = helper.userEnroll;
exports.newUserRegister = helper.newUserRegister;
exports.getConfigSetting = hfc.getConfigSetting;
exports.initNetworkTopo = helper.initNetworkTopo;
exports.getUserCert = helper.getUserCert;

exports.ErrorUserNotExists = helper.ErrorUserNotExists;

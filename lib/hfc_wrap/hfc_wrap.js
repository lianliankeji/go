...
var queryChaincode = function(peer, channelName, chaincodeName, args, fcn, username, org) {
	...
	var target = peer ? buildTarget(peer, org) : undefined
    var tx_id = null;
    ...
    .catch((err) => {
		logger.error('queryChaincode error: %s', err);
...
var invokeChaincode = function(peerNames, channelName, chaincodeName, fcn, args, username, org, eventHubTimeout) {
    ...
    .catch((err) => {
		logger.error('invokeChaincodeThe: Failed to send transaction due to error: %s', err);
...
var installChaincode = function(peers, chaincodeName, chaincodePath, chaincodeVersion, username, org) {
    ...
    .catch((err) => {
		logger.error('Failed to send install proposal due to error: %s', err);
        return Promise.reject(err);
	});
};
...
var instantiateChaincode = function(channelName, chaincodeName, chaincodeVersion, functionName, args, username, org, eventHubTimeout) {
    ...
    .then((results) => {
        logger.debug('instantiateChaincode results=', results)
		var proposalResponses = results[0];
		var proposal = results[1];
		var all_good = true;
		for (var i in proposalResponses) {
			...
			if (proposalResponses && proposalResponses[i].response && proposalResponses[i].response.status === 200) {
                ...
                logger.error('channel.sendInstantiateProposal proposalResponses err: %s', proposalResponses[i].details); //不要删除这个日志
			}
			all_good = all_good & one_good;
		}
		if (all_good) {
			...
			let txPromise = new Promise((resolve, reject) => {
				let handle = setTimeout(() => {
					eh.disconnect();
					reject(logger.errorf('The chaincode instantiate TxEvent timeout, peer=%s', eh.getPeerAddr()));
				}, eventHubTimeout);

				eh.registerTxEvent(deployId, (tx, code) => {
					logger.debug('The chaincode instantiate transaction has been committed on peer ' + eh.getPeerAddr());
					clearTimeout(handle);
					eh.unregisterTxEvent(deployId);
					eh.disconnect();

					if (code !== 'VALID') {
						reject(logger.errorf('The chaincode instantiate transaction was invalid, code = %s, peer=%s', code, eh.getPeerAddr()));
					} else {
						logger.debug('The chaincode instantiate transaction was valid.');
						resolve();
					}
				});
			});
            ...
            return Promise.reject('Failed to send instantiate Proposal or receive valid response. Response null or status is not 200.');
		}
	})
    .then((results) => {
        let resp = results[0];
		if (resp.status === 'SUCCESS') {
            ...
            let response = {
                success: true,
                message: 'Chaincode Instantiation is SUCCESS on ' + org,
                result: tx_id.getTransactionID()
            };
            return response;
		} else {
    ...
    .catch((err) => {
		logger.error('Failed to send instantiate due to error: %s', err);
		return Promise.reject(err)
	});
};
...
var createChannel = function(channelName, channelConfigPath, username, orgName) {
	...
	return helper.getOrgAdmin(orgName)
        ...
        .catch((err) => {
            return Promise.reject(logger.errorf('createChannel end Failed, err: %s ' + err));
        });
};
...
var joinChannel = function(channelName, peers, username, org, eventWaitTime) {
		...
		eventhubs.forEach((eh) => {
			let txPromise = new Promise((resolve, reject) => {
				...
				let blkRegNum = eh.registerBlockEvent((block) => {
					clearTimeout(handle);
                    en.unregisterBlockEvent(blkRegNum);
                    eh.disconnect();
    ...
    .catch((err) => {
        logger.error('joinChannel, catch error: %s ', err)
		return  Promise.reject(err)
	});
};

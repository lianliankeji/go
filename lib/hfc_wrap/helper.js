		...
		return client.getUserContext(username, true).then((user) => {
			if (user && user.isEnrolled()) {
				...
				return caClient.enroll({
					...
					return Promise.reject(logger.errorf('getAdminUser Failed, Error: %s' + err));
				});
			}
		});
	});
...
var registerAndEnroll = function(username, userOrg, isJson) {
	var member;
	var client = getClientForOrg(userOrg);
            ...
            if (user && user.isEnrolled()) {
                ...
                .then((adminUserObj)=> {
                        ...
                        .then((secret)=>{
                            ...
                            (err)=>{
                                return Promise.reject(logger.errorf(username + ' failed to register, err=%s', err));
                            })
                ...
                .then((secret) => {
                        ...
                        (err)=>{
                            return Promise.reject(logger.errorf(username + ' failed to enroll, err=%s', err));
                        })
                    })
                .then((message) => {
                        if (message && typeof message === 'string' && message.includes(
                                'Error:')) {
                            logger.error(username + ' enrollment failed, err=%s', err);
                            return message;
                        }
    ...
    .then((user) => {
        ...
        (err) => {
            logger.error('Failed to registerAndEnroll, user: %s, error: %s', username, err);

...
/*
注册接口需要防止同一个用户并发注册。如同时调用两次对同一个用户的注册，ca会为两次注册生成不同的secret， 在enroll时会报错
*/
var userRegLock = {}
function newUserRegister(username, userOrg, attrs) {
    if (userRegLock[username] == undefined) {
        userRegLock[username] = 1
    } else {
        return Promise.reject(logger.errorf('newUserRegister: user(%s) is registing.', username));
    }
    ...
    .then((adminUserObj)=> {
            ...
            let regReq = {
                enrollmentID: username,
                //enrollmentSecret: '',   //不指定secret，由系统自动生成
                role: 'client',
                attrs: attrs,
            ...
            .then((secret)=>{
                    delete userRegLock[username]
                    return secret
                },
                (err)=>{
                    return Promise.reject(logger.errorf('newUserRegister: %s failed to register, err=%s', username, err));
                 })
        },
        (err)=>{
            return Promise.reject(logger.errorf('newUserRegister: getCaAdminUser failed, err=%s', err));
        })
    .catch((err)=>{
        delete userRegLock[username]
        return Promise.reject(err)
...
function userEnroll(username, userOrg, secret) {
    var caClient = caClients[userOrg];
    ...
    .then((message) => {
        ...
        (err)=>{ 
            return Promise.reject(logger.errorf('userEnroll: %s failed to enroll, err=%s', username, err));
        })
    .catch((err)=>{
        return Promise.reject(err)
    })
...
function newUserRegisterAndEnroll(username, userOrg, attrs) {
    return newUserRegister(username, userOrg, attrs)
    .then((secret) => {
            logger.debug(username + ' registered successfully');
            return userEnroll(username, userOrg, secret)
        })
    .catch((err)=>{
        return Promise.reject(err)
    })
...
var ErrorUserNotExists = new Error("user not exists")
function getUserClient(username, userOrg, newUserWhenNone, attrs) {
    ...
    .then((user) => {
            if (user && user.isEnrolled()) {
                ...
                logger.debug('getUserClient: getUserContext for usr(%s) nil', username)
                return Promise.reject(ErrorUserNotExists);
            }
        },
        (err)=>{  // getUserContext err
...
exports.ErrorUserNotExists = ErrorUserNotExists;

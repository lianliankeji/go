...
func (t *KD) Init(stub shim.ChaincodeStubInterface, ifas *BaseInitArgs) ([]byte, error) {
	...
	if function == "init" {
		...
		*/
		...
		err = stub.PutState(t.getGlobalRackAllocRateKey(), eapJson)
		if err != nil {
		...
		*/
		...
		err = stub.PutState(t.getGlobalRackEncourageScoreCfgKey(), sercJson)
		if err != nil {
		...
		err = stub.PutState(t.getGlobalRackFinancCfgKey(), rfcJson)
		if err != nil {
			return nil, kdlogger.Errorf("Init PutState_Ex(rfc) error, err=%s.", err)
		}

		//虚拟一个超级账户，设置货币发行总额，给央行发行货币。
		err = Base.setIssueAmountTotal(stub, 10000000000, initTime)
		if err != nil {
			return nil, kdlogger.Errorf("Init setIssueAmountTotal error, err=%s.", err)
		}
...
func (t *KD) Invoke(stub shim.ChaincodeStubInterface, ifas *BaseInvokeArgs) ([]byte, error) {
	...
	var invokeTime = ifas.InvokeTime
	var accountEnt = ifas.AccountEnt

	if accountEnt == nil {
		accountEnt, err = Base.getAccountEntity(stub, accName)
		if err != nil {
			return nil, kdlogger.Errorf("Invoke getAccountEntity failed. err=%s", err)
		}

		ifas.AccountEnt = accountEnt
	}

	if function == "transefer2" { //带交易密码功能
		...
		if setPwd {
			...
			if !ok {
				return nil, kdlogger.Errorf("Invoke(transeferUsePwd): passwd invalide.")
			}
		} else {
		...
		_, err = Base.transferCoin(stub, accName, toAcc, transType, description, transAmount, invokeTime, sameEntSaveTransFlag)
		if err != nil {
		...
		err = stub.PutState(stateKey, eapJson)
		if err != nil {
		...
		err = stub.PutState(stateKey, rfcJson)
		if err != nil {
		...
		//其它函数看是否是query函数
		return t.Query(stub, ifas)
	}
}

// Query callback representing the query of a chaincode
func (t *KD) Query(stub shim.ChaincodeStubInterface, ifas *BaseInvokeArgs) ([]byte, error) {
	...
	if function == "queryRackAlloc" {
		...
		kdlogger.Debug("transPreCheck: accountEnt=%+v", accountEnt)
		//先看密码是否正确
		if len(pwd) > 0 {
			setPwd, err := t.isSetAccountPasswd(stub, accName)
			if err != nil {
		...
		if err != nil {
			return nil, kdlogger.Errorf("transPreCheck: getAccountLockedAmount(id=%s) failed. err=%s", accName, err)
		}
		...
		if err != nil {
			return nil, kdlogger.Errorf("transPreCheck: isEntityExists(%s) failed. err=%s", toAcc, err)
		}
		if !exists {
...
func (t *KD) setAllocEarnTx(stub shim.ChaincodeStubInterface, rackid, allocKey string, totalAmt int64,
	...
	err = stub.PutState(txKey, eatJson)
	if err != nil {
...
func (t *KD) setOneAccAllocEarnTx(stub shim.ChaincodeStubInterface, accName, txKey string) error {
	...
	err = stub.PutState(accTxKey, newTxsB)
	if err != nil {
...
func (t *KD) getAllocTxRecdByKey(stub shim.ChaincodeStubInterface, rackid, allocKey string) ([]byte, error) {
	...
	//先获取当前最大的序列号
	maxSeq, err := Base.getTransSeq(stub, seqKey)
	if err != nil {
...
func (t *KD) setRackEncourageScoreCfg(stub shim.ChaincodeStubInterface, rackid, cfgStr string, invokeTime int64) ([]byte, error) {
	...
	err = stub.PutState(stateKey, sepcJson)
	if err != nil {
...
func (t *KD) allocEncourageScore(stub shim.ChaincodeStubInterface, rrs *RackRolesEncourageScores, transFromAcc, transType, transDesc string, invokeTime int64, sameEntSaveTx bool, sellerComps int64) error {
	...
	_, err = Base.transferCoin(stub, transFromAcc, rrs.SellerAcc, transType, transDesc,
	...
	_, err = Base.transferCoin(stub, transFromAcc, rrs.FielderAcc, transType, transDesc,
	...
	_, err = Base.transferCoin(stub, transFromAcc, rrs.DeliveryAcc, transType, transDesc,
	...
	_, err = Base.transferCoin(stub, transFromAcc, rrs.PlatformAcc, transType, transDesc,
...
func (t *KD) userBuyFinance(stub shim.ChaincodeStubInterface, accName, rackid, fid, payee, transType, desc string, amount, invokeTime int64, sameEntSaveTx, isRenewal bool) ([]byte, error) {
	...
	//用户给融资方转账
	if !isRenewal {
		_, err = Base.transferCoin(stub, accName, payee, transType, desc, amount, invokeTime, sameEntSaveTx)
		if err != nil {
	...
	err = stub.PutState(rackFinacInfoKey, rfiJson)
	if err != nil {
		return nil, kdlogger.Errorf("userBuyFinance:  PutState failed. err=%s.", err)
	}

	err = stub.PutState(rackInfoKey, riJson)
	if err != nil {
		return nil, kdlogger.Errorf("userBuyFinance:  PutState failed. err=%s.", err)
	}

	err = stub.PutState(fiacInfoKey, fiJson)
	if err != nil {
		return nil, kdlogger.Errorf("userBuyFinance:  PutState failed. err=%s.", err)
	}
...
func (t *KD) setCurrentFid(stub shim.ChaincodeStubInterface, currentFid string) error {
	...
	err = stub.PutState(RACKFINACHISTORY_KEY, hisB)
	if err != nil {
...
func (t *KD) financeIssueFinishAfter(stub shim.ChaincodeStubInterface, currentFid string, invokeTime int64) error {
	...
	if finishIdB == nil {
		err = stub.PutState(RACKFINACISSUEFINISHID_KEY, []byte(currentFid))
		if err != nil {
...
func (t *KD) payUserFinance(stub shim.ChaincodeStubInterface, accName, reacc, rackid string, invokeTime int64, transType, desc string, sameEntSaveTx bool) error {
	...
	_, err = Base.transferCoin(stub, accName, reacc, transType, desc, totalAmt, invokeTime, sameEntSaveTx)
	if err != nil {
...
func (t *KD) getAccountRackInvestInfo(stub shim.ChaincodeStubInterface, accName string) (*AccRackInvest, error) {
	arfiB, err := stub.GetState(t.getAccountRackInvestInfoKey(accName))
	if err != nil {
...
func (t *KD) setAccountRackInvestInfo(stub shim.ChaincodeStubInterface, accRackInvest *AccRackInvest) error {
	...
	err = stub.PutState(t.getAccountRackInvestInfoKey(accName), ariB)
	if err != nil {
...
func (t *KD) setAccountPasswd(stub shim.ChaincodeStubInterface, accName, pwd string) error {
	...
	hash, err := baseCrypto.GenCipher(pwd, salt[:])
	if err != nil {
		return kdlogger.Errorf("setAccountPasswd: GenCipher failed.err=%s, acc=%s", err, accName)
	}

	err = stub.PutState(t.getUserCipherKey(accName), hash)
	if err != nil {
...
func (t *KD) authAccountPasswd(stub shim.ChaincodeStubInterface, accName, pwd string) (bool, error) {
	...
	cipher, err := stub.GetState(t.getUserCipherKey(accName))
	if err != nil {
	...
	ok, err := baseCrypto.AuthPass(cipher, pwd)
	if err != nil {
...
func (t *KD) decodeAccountPasswd(pwdBase64 string) (string, error) {
	...
	pwdB, err := baseCrypto.AESDecrypt(256, []byte(PWD_ENCRYPT_KEY), []byte(PWD_ENCRYPT_IV), pwdEncrypt)
	if err != nil {
...
func (t *KD) dateConvertWhenLoad(stub shim.ChaincodeStubInterface, srcCcid, key string, valueB []byte) (string, []byte, error) {
	...
	if srcCcid == "kd1.0" {
		if strings.HasPrefix(key, ACC_ENTITY_PREFIX) {
			...
			err = stub.PutState(t.getUserCipherKey(oldEnt.EntID), oldEnt.Cipher)
			if err != nil {

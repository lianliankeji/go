...
var logger = common.createLog("retailtest")
logger.setLogLevel(logger.logLevel.DEBUG)



process.on('exit', function (){
  logger.info(" ****  retail exit ****");
});

loulan.SetModuleName('retail')

loulan.SetLogger(logger)
loulan.UseDefaultRoute()


loulan.SetRequireParams(loulan.routeHandles.All,    {channame: 'retail-channel', org: 'org1'})
...
loulan.SetRequireParams(loulan.routeHandles.Query,      {exportInvokeIntf: '1', signature: 'LQ==', peer: 'peer0'})   //测试链上不验证签名，不会发送这个参数，这里自动加一个  'LQ=='为'-'的base64编码

loulan.SetRequireParams(loulan.routeHandles.Invoke,     {exportInvokeIntf: '1', signature: 'LQ==', peers: 'peer0,peer1,peer2,peer3'}) //测试链上不验证签名，不会发送这个参数，这里自动加一个 'LQ=='为'-'的base64编码

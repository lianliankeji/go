...
func (t *Template) Init(stub shim.ChaincodeStubInterface) (pbResponse pb.Response) {
	...
	tmpltLogger.Debugf("func =%s, args = %+v", function, args)
	...
	tmpltLogger.Debugf("initTime= %+v", initTime)
...
func (t *Template) Invoke(stub shim.ChaincodeStubInterface) (pbResponse pb.Response) {
	...
	if function != "account" {
		...
		if userName != accountEnt.Owner {
			return shim.Error(fmt.Sprintf("Invoke: username(%s) not match accout entity.", userName))
		}

		//非account时签名为最后一个参数
		sign, msg, err := t.getSignAndMsg(stub, len(args)-1)
		if err != nil {
			return shim.Error(fmt.Sprintf("Invoke: getSignAndMsg(%s) failed, err=%s.", accName, err))
		}

		//校验修改Entity的用户身份，只有Entity的所有者才能修改自己的Entity
		if err = t.verifyIdentity(stub, userName, sign, msg, accountEnt.OwnerPubKeyHash, accountEnt.OwnerIdentityHash); err != nil {
	...
	if function == "account" {
		...
		//签名为倒数第二个参数
		sign, msg, err := t.getSignAndMsg(stub, len(args)-2)
		if err != nil {
			return shim.Error(fmt.Sprintf("Invoke(account): getSignAndMsg(%s) failed, err=%s.", accName, err))
		}

		//校验修改Entity的用户身份，只有Entity的所有者才能修改自己的Entity
		if err = t.verifyIdentity(stub, userName, sign, msg, userPubKeyHash, userIdHash); err != nil {
		...
		err = stub.PutState("XXX", []byte(args[fixedArgCount]))
		if err != nil {
		...
		err = stub.PutState("YYY", []byte(args[fixedArgCount]))
		if err != nil {
...
func (t *Template) Query(stub shim.ChaincodeStubInterface, invokeTime int64) ([]byte, error) {
	...
	tmpltLogger.Debugf("queryTime= %+v", queryTime)
...
func (t *Template) newAccount(stub shim.ChaincodeStubInterface, accName string, accType int, userName, userIdHash, userPubKeyHash string, times int64, isCBAcc bool) ([]byte, error) {
	...
	tmpltLogger.Debugf("openAccount success: %+v", ent)
...
func (t *Template) getSignAndMsg(stub shim.ChaincodeStubInterface, signIdx int) ([]byte, []byte, error) {
	var err error
...
func (t *Template) verifyIdentity(stub shim.ChaincodeStubInterface, userName string, sign, signMsg []byte, ownerPubKeyHash, ownerIdentityHash string) error {
